/**
 * This is the client-side entrypoint for your tRPC API. It is used to create the `api` object which
 * contains the Next.js App-wrapper, as well as your type-safe React Query hooks.
 *
 * We also create a few inference helpers for input and output types.
 */
import { httpBatchLink, loggerLink, createTRPCProxyClient, TRPCClientError } from '@trpc/client'
import { type inferRouterInputs, type inferRouterOutputs } from '@trpc/server'
import superjson from 'superjson'
import type { AppRouter } from 'nuxt-web'
import { isClient } from 'utils'
import { useTRPC } from 'use-trpc'

const getBaseUrl = () => {
  if (isClient()) return '' // browser should use relative url
  return `http://localhost:${process.env.PORT ?? 3000}` // dev SSR should use localhost
}

const getConfig = () => ({
  /**
   * Transformer used for data de-serialization from the server.
   *
   * @see https://trpc.io/docs/data-transformers
   */
  transformer: superjson,

  /**
   * Links used to determine request flow from client to server.
   *
   * @see https://trpc.io/docs/links
   */
  links: [
    loggerLink({
      enabled: (opts) =>
        process.env.NODE_ENV === 'development' || (opts.direction === 'down' && opts.result instanceof Error),
    }),
    httpBatchLink({
      url: `${getBaseUrl()}/api/trpc`,
    }),
  ],
})

/** A set of type-safe react-query hooks for your tRPC API. */
export const initDefApi = () => ({
  config() {
    return getConfig()
  },
  /**
   * Whether tRPC should await queries when server rendering pages.
   *
   * @see https://trpc.io/docs/nextjs#ssr-boolean-default-false
   */
  ssr: false,
})

let clientApi: ReturnType<typeof createTRPCProxyClient<AppRouter>>
export const getApi = () => {
  if (!clientApi) {
    clientApi = createTRPCProxyClient<AppRouter>(getConfig())
  }
  return clientApi
}

/**
 * Inference helper for inputs.
 *
 * @example type HelloInput = RouterInputs['example']['hello']
 */
export type RouterInputs = inferRouterInputs<AppRouter>

/**
 * Inference helper for outputs.
 *
 * @example type HelloOutput = RouterOutputs['example']['hello']
 */
export type RouterOutputs = inferRouterOutputs<AppRouter>

export function isTRPCClientError(error: unknown): error is TRPCClientError<AppRouter> {
  return error instanceof TRPCClientError
}

export const useTrpc = () => {
  return useTRPC<AppRouter>({
    // wsUrl: `ws://localhost:8080/`,
    client: getConfig(),
  })
}
